...
export function ToolSelector() {
  ...
  return (
    <div className="px-6 py-6">
      <p className="text-xs font-semibold uppercase tracking-widest text-[var(--muted-foreground)] mb-4 px-1">
        Tools
      </p>
        ...
        {AVAILABLE_TOOLS.map(({ id, label, icon: Icon }) => {
          ...
          return (
            <button
              key={id}
              onClick={() => toggle(id)}
              className="w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-base transition-all text-left hover:bg-[var(--accent)] cursor-pointer"
            >
